...
class UpdateWorkerRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        foreach (request()->document_name as $index => $documentName) {
            $rules['certificate.'.$index] = 'required_without:old_certificate_url.'.$index .'|max:2048|mimes:pdf,jpeg,jpg,png';
            $rules['dates.'.$index] = 'nullable|date_format:Y-m-d';
        }
        foreach (request()->admin_document_name as $index => $adminDocumentName) {
            $rules['admin_certificate.'.$index] = 'required_without:old_admin_certificate_url.'.$index .'|max:2048|mimes:pdf,jpeg,jpg,png';
    ...
    public function messages()
    {
        ...
        foreach (request()->admin_document_name as $index => $documentName) {
            $messages['admin_certificate.'.$index . '.required_without'] =  __('customvalidation.worker.certificate.required_without');
            ...
            $messages['admin_dates.'.$index . '.required'] =  __('customvalidation.worker.dates.required');
            $messages['admin_dates.'.$index . '.date_format'] =  __('customvalidation.worker.dates.format');
        }
        foreach (request()->document_name as $index => $documentName) {
            $messages['certificate.'.$index . '.required_without'] =  __('customvalidation.worker.certificate.required_without');
